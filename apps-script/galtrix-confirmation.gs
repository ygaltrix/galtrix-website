...
function listLeads() {
  ...
  const out = data.map(function (row) {
    ...
    // Don't leak the token to the dashboard
    delete o.token;
    return o;
  });
  // Newest first
    ...
    '<div style="background:#04050d;padding:40px 16px;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Inter,Arial,sans-serif;">' +
      '<table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:560px;margin:0 auto;background:#06081a;border:1px solid rgba(34,211,238,0.18);border-radius:24px;overflow:hidden;">' +
        ...
        '<tr><td style="padding:0 36px 28px 36px;border-top:1px solid rgba(148,163,184,0.10);padding-top:20px;">' +
          '<p style="margin:0;font-size:12px;line-height:1.6;color:#7b86a8;">If the button doesn&rsquo;t work, copy &amp; paste this link into your browser:<br><span style="color:#67e8f9;word-break:break-all;">' + escapeHtml(verifyUrl) + '</span></p>' +
        '</td></tr>' +
...
function htmlPage(state, name, extra) {
  ...
  switch (state) {
    case 'success':
      ...
      body = 'Thanks ' + greet + '. Your inquiry is now on its way to our team. We\u2019ll be in touch within 24 to 48 hours.';
      break;
    case 'already':
      accent = 'cyan';
      title = 'Already verified — GALTRIX';
      headline = 'This email is already verified.';
      body = 'Looks like you already confirmed this inquiry, ' + greet + '. There\u2019s nothing more to do — we\u2019ll be in touch shortly.';
      break;
    case 'expired':
      accent = 'amber';
    ...
    case 'invalid':
      ...
      body = 'The link doesn\u2019t match any inquiry on file. Please submit your inquiry again from the GALTRIX website.';
      break;
    case 'not-found':
      accent = 'amber';
      title = 'Inquiry not found — GALTRIX';
      headline = 'We couldn\u2019t find this inquiry.';
      body = 'The reference in this link doesn\u2019t exist. Please submit your inquiry again from the GALTRIX website.';
      break;
    case 'error':
    default:
      accent = 'red';
  ...
  const html =
    ...
    '<div class="wrap"><div class="card">' +
      ...
      '<div class="foot">Built for what\u2019s next.</div>' +
    '</div></div></body></html>';
